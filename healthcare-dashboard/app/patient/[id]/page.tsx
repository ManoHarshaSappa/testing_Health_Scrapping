...
export default function PatientPage() {
  ...
  useEffect(() => {
    fetch(`/api/patient/${id}`).then(r => r.json()).then(d => {
      setPatient(d.patient); setVisits(d.visits); setLoading(false);
    });
  }, [id]);
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-[#05091a]">
      <div className="text-center">
        ...
        <p className="text-blue-400 text-sm">Loading patient data...</p>
      </div>
    </div>
  );
  if (!patient) return (
  ...
  const historyText = visits.map(v =>
          ...
          {consulting ? (
            <div className="flex-1 flex flex-col p-8 overflow-hidden">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-400/60 text-xs mt-0.5">Aria (AI) is speaking with {patient.NAME}</p>
                </div>
              ...
              <div className="flex-1 overflow-hidden">
                <VoiceAgent patientName={patient.NAME} patientHistory={historyText || "No prior history on record."} />
              </div>
            </div>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
              ...
              <p className="text-blue-400/60 text-sm mb-8 max-w-xs">
                Aria will greet {patient.NAME} and collect their symptoms automatically.
              </p>
              <button onClick={() => setConsulting(true)}
